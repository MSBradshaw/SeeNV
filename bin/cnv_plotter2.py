...
def get_args():
    ...
    parser.add_argument('--all_calls',
                        dest='all_calls',
                        help='file with path with a BGZIPED and TABIXED Savvy calls set file path on each line')
...
def plot_coverage_stats(_ax, _args, _call_colors, _non_samp_pos, _non_samp_xs, _regions_pos, _means, _stdevs, _xs, _chrom, _num_overlapped_probes):
    if _args.title:
        if ';' in _args.title:
            ...
            _ax.set_title(_args.title + ' # Probes: ' + str(_num_overlapped_probes), loc='right', fontsize=6)
    ...
    _ax.set_title('Z-score', fontsize=4, x=0.0, fontdict={'horizontalalignment': 'left'})
    ...
    if _args.all_calls:
        ...
        for i,_calls in enumerate(_all_calls):
            ...
            for _call in _calls:
                ...
                # do not plot all the way at the bottom so that probe confidence colors are not covered at all, this could be a problem if there are close to 10 calls in the window
                if _ymin == 0 and _number_of_calls_to_plot < 7:
                    _ymin = 0.1
...
def man_plot(axs, infile, target):
    ...
    axs[0].set_title('Mean', fontsize=4, x=0.0, fontdict={'horizontalalignment': 'left'})
    axs[1].set_title('Stdev', fontsize=4, x=0.0, fontdict={'horizontalalignment': 'left'})
...
def main():
    ...
    call_target_region = utils.Interval(chrom=chrom,start=max(0,int(args.region.split(':')[1].split('-')[0])), end=int(args.region.split(':')[1].split('-')[1]),data=None)
    ...
    exons = utils.get_intervals_in_region(target_region, args.exons)

    overlaped_probes = utils.get_intervals_in_region(call_target_region,args.exons)
    ...
    ax, lns1, lns2b, lns2, all_calls, savvy_calls, xmin, xmax, target_interval = plot_coverage_stats(ax, args, call_colors, non_samp_pos, non_samp_xs, regions_pos, means, stdevs, xs, chrom, len(overlaped_probes))
    ...
    if args.gnomad_sv:
        ...
        axs[gnomad_idx].set_title('gnomAD SV AF', fontsize=4, x=0.0, fontdict={'horizontalalignment': 'left'})
    ...
    if args.exons:
        ...
        probe_scores = [ float(x.data[-1]) for x in exons] 
        print('EXONS!')
        for x in exons:
            print(x)
            print(x.data)
        ...
        print('Lengths!!!!')
        print(len(axs[last_reg_plt_idx].xaxis.get_ticklines()))
        print(len(probe_scores))
        #[t.set_color(plt.cm.RdYlGn(probe_scores[i])) for i,t in enumerate(axs[last_reg_plt_idx].xaxis.get_ticklines())]
        ...
        print(call_set)
        axs[num_calls_idx].set_title('Num. Calls', fontsize=4, x=0.0, fontdict={'horizontalalignment': 'left'}) 
    ...
    # add the probe qualities to the bottom of the first plot
    y_min = ax.get_ylim()[0]
    y_range = ax.get_ylim()[1] + y_min
    y_min = y_min + (.3 * y_range)
    ax.scatter(tick_xs,[y_min] * len(tick_xs),c=probe_scores,cmap ='RdYlGn',s=1, marker='|', label='Probe Confidence')
    #for i in range(len(tick_xs)):
    #    print(tick_xs[i])
    #    print(probe_scores[i])
    #    probe_scores # float values to color points with
    #    ax.scatter(tick_xs[i],y_min,c=probe_scores[i],cmap ='RdYlGn',s=1, marker='_', label='Probe Confidence')
