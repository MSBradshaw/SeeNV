...
def get_args():
    ...
    parser.add_argument('--site_quality',
                        dest='site_quality',
                        type=int,
                        default=-1,
                        help='column index in the sites file that contains the site quality information, -1 means ignore it')
...
def get_calls(_files: typing.List[str], _region: utils.Interval, _with_sample_id: str = None,
    ...
    for _f in _files:
        ...
        try:
            ...
            warnings.warn('Warning: no overlapping calls in ' + _f)
            continue
...
def mark_calls(_ax: plt.Axes, _ax_legend: plt.Axes, _calls: typing.List[utils.Interval], _hatch_map: typing.Dict,
    ...
    for _i, _c in enumerate(_calls):
        ...
        if 'Dup' in _c.data[0] or 'DUP' in _c.data[0] or 'dup' in _c.data[0]:
            ...
            warnings.warn('Warning! It is not a DUP or DEL:' + _c.data[0])
        ...
        if _sites_tabix is not None:
            ...
            for _line in res:
                ...
                if _colors[-1] not in _color_legend:
                    _color_legend[_colors[-1]] = ss(_line)[_site_quality]
                _added = True
                break
    ...
    for _line in _res:
        ...
        try:
            if 'repeatElement' in _row[4]:
                ...
                warnings.warn('Unrecognized {}'.format(_row[4]))
        except IndexError:
            warnings.warn('Warning IndexError repressed' + str(_row))
    _colors_index = 0
...
def probe_quality_heatmap(_ax: plt.Axes, _ax_legend: plt.Axes, _file: str, _region: utils.Interval,
    ...
    for i in range(100):
        ...
        for _line in _res:
            # if there are fewer than 4 things in the row, it is impossible for it to have a quality
            if len(ss(_line)) < 4:
                continue
...
def main():
    ...
    probe_quality_heatmap(_ax=heatmap_ax, _ax_legend=heatmap_legend_ax, _file=args.sites, _region=target_window,
                          _x_range=(target_window.start, target_window.end),_site_quality=args.site_quality)
    ...
    """
    Make a legendary legend
    """
    main_legend(first_legend, target_window, args.coverage_scores, input_file_hatch_map, args.sites)
